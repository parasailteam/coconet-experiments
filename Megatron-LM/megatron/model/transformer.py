...
print("execution_type: '%s', layernorm_execution_type: '%s'"%(execution_type,layernorm_execution_type))
...
class ParallelMLP(MegatronModule):
    ...
    def __init__(self, mlp_activation_func, init_method,
        ...
        # print(83, "LINE", args.hidden_size)
        # Project back to h.
...
class ParallelTransformerLayer(MegatronModule):
    ...
    def forward(self, hidden_states, attention_mask, layer_past=None,
        ...
        if ((self.layer_number == 0 and layernorm_execution_type != 'REPEATED') or layernorm_execution_type == 'REPEATED'):
            # timers("LayerNorm").start()
            layernorm_output = self.input_layernorm(hidden_states)
            # torch.distributed.coconetLayerNorm(hidden_states, hidden_states.shape[2])
        ...
        if self.apply_residual_connection_post_layernorm:
            ...
            if execution_type == 'AR-C':
                ...
                layernorm_input = self.attention(layernorm_output,
                            ...
                            layer_past=layer_past,
                            get_key_value=get_key_value)
        # print(layernorm_input.shape)
        # Layer norm post the self attention.
        if (layernorm_execution_type == 'REPEATED'):
            # timers("MatMul").start()
        ...
        # MLP.
        if execution_type == 'AR-C':
            mlp_output = self.mlp(layernorm_output,add_ten=None)
        ...
        if self.apply_residual_connection_post_layernorm:
            if execution_type == 'AR-C':
                ...
                output = self.mlp(layernorm_output, add_ten=layernorm_output)
