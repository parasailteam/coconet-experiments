...
import time
import os 
gemm_allreduce_fusion = os.environ.get("GEMM_ALLREDUCE_FUSION", '0') == '1'
...
def parallel_lm_logits(input_, word_embeddings_weight, parallel_output,
    ...
    if DEBUG_INFO:
        print(f"gpu {torch.cuda.current_device()} lm logits comp start{time.time()}\n")
    # Matrix multiply.
    if bias is None:
    ...
    if DEBUG_INFO:
        print(f"gpu {torch.cuda.current_device()} lm logits comp end{time.time()}\n")
    # Gather if needed.
    if parallel_output:
        return logits_parallel
    if DEBUG_INFO:
        print(f"gpu {torch.cuda.current_device()} lm logits comm start {time.time()}\n")
    if gemm_allreduce_fusion:
        output = logits_parallel
    else:
        output = mpu.gather_from_tensor_model_parallel_region(logits_parallel)
    if DEBUG_INFO:    
        torch.cuda.synchronize()
        print(f"gpu {torch.cuda.current_device()} lm logits comm end {time.time()}\n")
    return output
