...
# change layout and .contious() may introduce extra overhead.
def reduce_scatter_tensor_into_1d_tensor(tensor):
    """Break a tensor into equal 1D chunks."""
    world_size = get_tensor_model_parallel_world_size()
    data = tensor.view(-1)
    partition_size = torch.numel(data) // get_tensor_model_parallel_world_size()
    chunks = [data[i*partition_size:(i+1)*partition_size] for i in range(world_size)]

    reduce_scattered = torch.empty(partition_size, dtype=tensor.dtype,
                                    device=torch.cuda.current_device(),
                                    requires_grad=False)
    #start_index = partition_size * get_tensor_model_parallel_rank()
    #end_index = start_index + partition_size
    torch.distributed.reduce_scatter(reduce_scattered, chunks, group=get_tensor_model_parallel_group())
    return reduce_scattered
