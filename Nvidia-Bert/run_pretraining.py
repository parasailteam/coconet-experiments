...
# Olli: These didn't seem to be available in our pytorch version
...
def parse_arguments():
    ...
    parser.add_argument('--fused_allreduce',
                        default=False,
                        action='store_true',
                        help="Whether to do fused allreduces during gradient accumulation steps.")
    parser.add_argument('--fused_allreduce_fp16',
                        default=False,
                        action='store_true',
                        help="Whether to do fused allreduces during gradient accumulation steps.")
...
def setup_training(args):
    ...
    if args.local_rank == -1:
        ...
        print (world_size, rank, device, args.local_rank)
    ...
    if is_main_process():
        # Olli: disabled the JSON backend due to some error it encountered
        # dllogger.init(backends=[dllogger.JSONStreamBackend(verbosity=dllogger.Verbosity.VERBOSE,
        #                                                    filename=args.json_summary),
        #                         dllogger.StdOutBackend(verbosity=dllogger.Verbosity.VERBOSE, step_format=format_step)])
        dllogger.init(backends=[dllogger.StdOutBackend(verbosity=dllogger.Verbosity.VERBOSE, step_format=format_step)])
    else:
...
def prepare_model_and_optimizer(args, device):
    ...
    if args.fused_allreduce_fp16:
        global params_fp32
        params_fp32 = [p.detach().clone().to(device) for p in model.parameters()]
        # wait_for_input(439) 9.9GB here
        model.half()
        model.to(device)
        # wait_for_input(417) 14 GB here
        global params_fp16
        params_fp16 = [p for p in model.parameters()]
    else:
        model.to(device)
    ...
    if args.local_rank != -1:
        if not args.allreduce_post_accumulation and not args.fused_allreduce:
    ...
    #wait_for_input(485) 14.6 GB

    criterion = BertPretrainingCriterion(config.vocab_size)

    return model, optimizer, lr_scheduler, checkpoint, global_step, criterion

fused_allreduce_state = defaultdict(lambda: {})
r_storage_buff = None
flat_raw = None
...
#        torch.distributed.all_reduce(flat_raw)
        ...
        if args.fp16:
            ...
            had_overflow = 0
        global_step += 1 # Always update to make sure the perf test ends
        ...
        for param in model.parameters():
            param.grad = None
    elif args.fused_allreduce:
        #Test to see the GPU memory usage when flattening the grads
        # master_grads = [p.grad for p in amp.master_params(optimizer) if p.grad is not None]
        # if flat_raw is None:
        #   flat_grad_size = sum(p.numel() for p in master_grads)
        #   allreduce_dtype = torch.float16 if args.allreduce_post_accumulation_fp16 else torch.float32
        ...
        if args.fused_allreduce_fp16:
            ...
            grads = [p.grad for p in params_fp16]
            
            overflow_buf.zero_()
            if args.use_adam:
#                torch.distributed.adam_update(params_fp32, params_fp16, grads,
#                   lr, beta1, beta2,
#                    1 / (get_world_size() * args.gradient_accumulation_steps),
#                    overflow_buf)
                pass
            else:
#                torch.distributed.lamb_update(params_fp32, params_fp16, grads,
#                    lr, beta1, beta2,
#                    1 / (get_world_size() * args.gradient_accumulation_steps),
#                    overflow_buf)
                pass
            num_overflows = overflow_buf.item() 
            if num_overflows > 0 and is_main_process():
                print('overflow', num_overflows, flush=True)
            ...
            if args.use_adam:
                torch.distributed.adam_update(params, grads, lr, beta1, beta2)

            else:
                torch.distributed.lamb_update(params, grads, lr, beta1, beta2)
            had_overflow = 0
        global_step += 1 # Always update to make sure the perf test ends
        # Zero gradients if the kernel doesn't already do it
        # TODO: add zeroing to all kernel variants
        # if not (args.use_adam and args.fused_allreduce_fp16):
        #     for param in model.parameters():    
        #         #param.grad = None
...
def main():
    ...
    print("parameters in model", sum([p.nelement() for p in model.parameters()]))
    if is_main_process():
    ...
    if args.do_train:
        ...
        with torch.autograd.profiler.emit_nvtx():
          while True:
            ...
            overflow_buf = None
            # TODO: for args.fused_allreduce with fp16 we'll need to create the overflow_buf here likely
            if args.allreduce_post_accumulation or (args.fused_allreduce and args.fused_allreduce_fp16):
                overflow_buf = torch.cuda.IntTensor([0])
            
            #wait_for_input(692) 14.6 GB
            for f_id in range(f_start_id + 1 , len(files)):
                ...
                train_iter = train_dataloader #tqdm(train_dataloader, desc="Iteration", disable=args.disable_progress_bar) if is_main_process() else train_dataloader

                if raw_train_start is None:
                    raw_train_start = time.time()
                time_iter = time.time()
                time_global = time.time()
                print(680, len(train_iter), len(files), f_start_id)
                #wait_for_input(712) 14.6GB
                for step, batch in enumerate(train_iter):
                    training_steps += 1
                    
                    if args.fused_allreduce_fp16:
                        batch = [t.half() if t.dtype == torch.float32 else t for t in batch]
                    
                    batch = [t.to(device) for t in batch]
                    #wait_for_input(722) 14,6GB
                    ...
                    if args.gradient_accumulation_steps > 1:
                        if not (args.allreduce_post_accumulation or args.fused_allreduce):
                            # this division was merged into predivision
                            loss = loss / args.gradient_accumulation_steps
                            divisor = 1.0
                    #wait_for_input(735) 17GB
                    if args.allreduce_post_accumulation or args.fused_allreduce_fp16:
                        if args.fp16:
                            with amp.scale_loss(loss, optimizer,
                                    delay_overflow_check=(args.allreduce_post_accumulation or args.fused_allreduce),
                                    delay_unscale=args.fused_allreduce) as scaled_loss:
                                scaled_loss.backward()
                        else:
                            loss.backward()

                    #wait_for_input(743) 22GB
                    average_loss += loss.item()

                    if training_steps % args.gradient_accumulation_steps == 0:
                        #torch.cuda.synchronize() # wait for backward pass to finish before starting timer on optimizer step
                        #torch.distributed.barrier()
                        lr_scheduler.step()  # learning rate warmup
                        #time_opt_start = time.time()
                        global_step = take_optimizer_step(args, optimizer, model, overflow_buf, global_step)
                        #torch.cuda.synchronize() # wait for any asynchronous operation to finish
                        #torch.distributed.barrier()
                        #time_opt_end = time.time()
                        #dur_opt = time_opt_end - time_opt_start
                        time_global_end = time.time()
                        dur_global = time_global_end - time_global
                        time_global = time_global_end
                        if is_main_process():
                            print('global step {} global step duration {}'.format(global_step, dur_global), flush=True)
                            #print('global step {} optimizer duration {} global step duration {}'.format(global_step, dur_opt, dur_global), flush=True)
                    # if global_step >= 10:
                    ...
                    if global_step >= args.steps_this_run or training_steps % (
                        ...
                        if False and is_main_process() and not args.skip_checkpoint:
                        ...
                        if global_step >= args.max_steps or global_step >= args.steps_this_run or timeout_sent:
                            print('Stopping the profiler', flush=True)
                            torch.cuda.profiler.cudart().cudaProfilerStop()
                    ...
                    dur_iter = time_iter_end - time_iter
                    time_iter = time_iter_end
                    if is_main_process():
                        print('step {} step duration {}'.format(training_steps, dur_iter), flush=True)
