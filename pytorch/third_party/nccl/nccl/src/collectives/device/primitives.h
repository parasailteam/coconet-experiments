...
class ncclPrimitives {
 private:
  ...
  template <int DIRECTRECV, int DIRECTSEND, int RECV, int SEND, int SRC, int DST, int WEIGHT_UPDATE>
  inline __device__ void
  GenericOp(const T* srcPtr, T* dstPtr, T* firstMoment, T* secondMoment, int nelem, ssize_t directOffset, const T alpha, 
            const T beta1, const T beta2, const int epoch) {
    int offset = 0;
      ...
      if (!syncThread) {
        ...
        if (realSize > 0) {
          ...
          if (DIRECTRECV && recvDirectBuff[0]) {
            ...
            if (SEND) {
              ReduceOrCopyMulti<UNROLL, FUNC, T, T, 1, 1, 1, NSEND, WEIGHT_UPDATE, 0,0>(tid, nthreads-WARP_SIZE, 1, srcs, nsend, dsts+1, firstMoment, secondMoment, nullptr, nullptr, realSize, alpha, beta1, beta2, 0.0f, epoch, 0, 0, 0, nullptr,nullptr, 0,nullptr);
            }
          } else {
            ReduceOrCopyMulti<UNROLL, FUNC, T, T, RECV+SRC, RECV*NRECV+SRC, SEND+DST, SEND*NSEND+DST, WEIGHT_UPDATE, 0,0>(tid, nthreads-WARP_SIZE, RECV*nrecv+SRC, srcs, SEND*nsend+DST, dsts, firstMoment, secondMoment, nullptr, nullptr, realSize, alpha, beta1, beta2,0.0f, epoch, 0, 0, 0, nullptr,nullptr, 0,nullptr);
          }
        }
      }
      barrier();
 ...
 public:
  ...
  send(const T* src, int nelem) {
    GenericOp<0, 0, 0, 1, 1, 0, 0>(src, NULL, NULL, NULL, nelem, 0, (T)0.0, (T)0.0, (T)0.0, (T)0.0);
  }
  __device__ __forceinline__ void
  directSend(const T* src, ssize_t directOffset, int nelem) {
    GenericOp<0, 1, 0, 1, 1, 0, 0>(src, NULL, NULL, NULL, nelem, directOffset, (T)0.0, (T)0.0, (T)0.0, (T)0.0);
  }

  __device__ __forceinline__ void
  recv(T* dst, int nelem) {
    GenericOp<0, 0, 1, 0, 0, 1, 0>(NULL, dst, NULL, NULL, nelem, 0, (T)0.0, (T)0.0, (T)0.0, (T)0.0);
  }
  __device__ __forceinline__ void
  directRecv(T* dst, ssize_t directOffset, int nelem) {
    GenericOp<1, 0, 1, 0, 0, 1, 0>(NULL, dst, NULL, NULL, nelem, directOffset, (T)0.0, (T)0.0, (T)0.0, (T)0.0);
  }

  __device__ __forceinline__ void
  copySend(const T* src, T* dst, int nelem) {
    GenericOp<0, 0, 0, 1, 1, 1, 0>(src, dst, NULL, NULL, nelem, 0, (T)0.0, (T)0.0, (T)0.0, (T)0.0);
  }
  __device__ __forceinline__ void
  directCopySend(const T* src, T* dst, ssize_t directOffset, int nelem) {
    GenericOp<0, 1, 0, 1, 1, 1, 0>(src, dst, NULL, NULL, nelem, directOffset, (T)0.0, (T)0.0, (T)0.0, (T)0.0);
  }

  __device__ __forceinline__ void
  recvCopySend(T* dst, int nelem) {
    GenericOp<0, 0, 1, 1, 0, 1, 0>(NULL, dst, NULL, NULL, nelem, 0, (T)0.0, (T)0.0, (T)0.0, (T)0.0);
  }
  __device__ __forceinline__ void
  directRecvCopySend(T* dst, ssize_t directOffset, int nelem) {
    GenericOp<1, 1, 1, 1, 0, 1, 0>(NULL, dst, NULL, NULL, nelem, directOffset, (T)0.0, (T)0.0, (T)0.0, (T)0.0);
  }

  __device__ __forceinline__ void
  recvReduceCopy(const T* src, T* dst, int nelem) {
    GenericOp<0, 0, 1, 0, 1, 1, 0>(src, dst, NULL, NULL, nelem, 0, (T)0.0, (T)0.0, (T)0.0, (T)0.0);
  }

  __device__ __forceinline__ void
  recvReduceSend(const T* src, int nelem) {
    GenericOp<0, 0, 1, 1, 1, 0, 0>(src, NULL, NULL, NULL, nelem, 0, (T)0.0, (T)0.0, (T)0.0, (T)0.0);
  }

  __device__ __forceinline__ void
  recvReduceCopySend(const T* src, T* dst, int nelem) {
    GenericOp<0, 0, 1, 1, 1, 1, 0>(src, dst, NULL, NULL, nelem, 0, (T)0.0, (T)0.0, (T)0.0, (T)0.0);
  }
  __device__ __forceinline__ void
  directRecvReduceCopySend(const T* src, T* dst, ssize_t directOffset, int nelem) {
    // Direct is only for the send part
    GenericOp<0, 1, 1, 1, 1, 1, 0>(src, dst, NULL, NULL, nelem, directOffset, (T)0.0, (T)0.0, (T)0.0, (T)0.0);
  }
  ...
  directRecvReduceCopySendAdam(const T* src, T* weight, T* firstMoment, T*secondMoment, 
    ...
    GenericOp<0, 1, 1, 1, 1, 1, 1>(src, weight, firstMoment, secondMoment, nelem, directOffset, alpha, beta1, beta2, epoch);
  }

  __device__ __forceinline__ void 
  directRecvReduceCopySendWeight(const T* src, T* weight, ssize_t directOffset, int nelem, const T alpha) {
    GenericOp<0, 1, 1, 1, 1, 1, 1>(src, weight, NULL, NULL, nelem, directOffset, alpha, (T)0.0, (T)0.0, (T)0.0);
  }
