...
def model_parallel_update(input_, bias, add_tensor, group=group.WORLD, async_op=False):
    _check_tensor_list(input_, "input")
    _check_tensor_list(bias, "bias")
    _check_tensor_list(add_tensor, "add_tensor")
    ...
    if group == GroupMember.WORLD:
        _check_default_pg()
        work = _default_pg.model_parallel_update(input_, bias, add_tensor, opts)
    else:
...
def overlap_matmul_allreduce_bias_dropout_layernorm(input_, w, bias, add_tensor, output, normalizationDimSize, group=group.WORLD, async_op=False):
    _check_tensor_list(input_, "input")
    _check_tensor_list(bias, "bias")
    _check_tensor_list(add_tensor, "add_tensor")
    ...
    if group == GroupMember.WORLD:
        _check_default_pg()
        work = _default_pg.overlap_matmul_allreduce_bias_dropout_layernorm(input_, w, bias, add_tensor, output, normalizationDimSize, opts)
    else:
        assert False, "not implemented"
...
def coconetLayerNorm(input_, normalizationDimSize, group=group.WORLD, async_op=False):  
    if _rank_not_in_group(group):
        return
    
    if group == GroupMember.WORLD:
        _check_default_pg()
        work = _default_pg.coconetLayerNorm(input_, normalizationDimSize)
    else:
        assert False, "not implemented"
...
def model_parallel_update_with_layer_norm(input_, bias, add_tensor, normalizationDimSize, group=group.WORLD, async_op=False):
    _check_tensor_list(input_, "input")
    _check_tensor_list(bias, "bias")
    _check_tensor_list(add_tensor, "add_tensor")
    ...
    if group == GroupMember.WORLD:
        _check_default_pg()
        work = _default_pg.model_parallel_update_with_layer_norm(input_, bias, add_tensor, normalizationDimSize, opts)
    else:
        assert False, "not implemented"
...
def adam_update(params, ms, vs, grads, alpha, group=group.WORLD, async_op=False):
    ...
    if group == GroupMember.WORLD:
        ...
        work = _default_pg.adam_update(
            params, ms, vs, grads, float(alpha), opts)
    else:
        assert False, "not implemented"
